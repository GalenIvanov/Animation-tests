...
context [
        ...
        collect/into [
            while [not tail? data][
                ...
                switch mode [
                    line [
                        repeat n seq - 1 [
                            keep sqrt data/:n/x - data/(n + 1)/x ** 2 + (data/:n/y - data/(n + 1)/y ** 2)
                        ]
                    ]
                    arc [keep arc-length data/2/x data/4]
                    bezier [
                        b-copy: copy/part data seq
                        keep last bezier-lengths b-copy 200
                    ]
                ]
                data: skip data seq
            ]
    ...
    line-carry: function [px py carry phi][
        collect [
    ...
    break-path: function [
        ...
        data  [block!]       {block generated by lengths function}
        seg-n [integer!]     {number of segments to break the path into}
        ...
        seg-len: len / seg-n
        print [
            "The length is" len newline
            "The number of segments is" seg-n newline
            "The segment length is" seg-len
        ]
        ...
        collect/into [
            ...
            while [not tail? data][
                ...
                switch mode [
                    line [
                        repeat n seq - 1 [
                            if carry > 0 [                         ; fraw the remaining part of a line
                                keep line-carry px py carry phi
                            ...
                            px: p1/x
                            py: p1/y
                            phi: arctangent2 data/(n + 1)/y - p1/y data/(n + 1)/x - p1/x
                    ...
                    arc [
                        ...
                        arc-b: data/3
                        arc-s: data/4
                        arc-len: arc-length arc-r/x arc-s
                        sign: pick [-1 1] arc-s < 0
                        phi: sign * arc-angle arc-r/x seg-len
                        if carry > 0 [   ; if there is a leftover from the previous primitives
                            c-angle: sign * arc-angle arc-r/x carry
                            print ["Leftover angle:" c-angle]
                            keep reduce ['pen (color) 'arc arc-c arc-r arc-b to-integer round c-angle]
                            arc-s: arc-s - c-angle
                            arc-b: arc-b + c-angle
                        ]
                        ...
                        if (absolute arc-s) > 0 [
                            ...
                            print [seg-len arc-r arc-s phi arc-length arc-r/x arc-s]
                            carry: seg-len - arc-len arc-r/x arc-s  ; set carry
                        ]
                        data-len: next data-len
                    ]    
                    bezier [
    ...
    stroke-path: [
        [
            ...
            path 
            set width integer!
            set color [tuple! | word!]
            ; line-cap
            ; value or from v1 to v2 ???
            (
                new-block: break-path path-block to-integer dur-v * frame-rate width color
            )
            keep (new-block)
        ]
        ()
